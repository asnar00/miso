...
When the app starts, it checks with the server to see if a newer build is available. If the app's build number is lower than the server's latest build number, a modal appears prompting the user to update. The check also runs when the app returns from the background.

## How it works

1. The app knows its own build number
2. The server knows the latest build number
3. On app launch (and when returning from background), the app asks the server for the latest build number
4. If the app's build is older, show an update prompt

## Test User Exemption

The version check is skipped for test users to avoid interrupting development:
- Email: test@example.com
- Name: asnaroo

This allows developers to continue using older builds during testing.
