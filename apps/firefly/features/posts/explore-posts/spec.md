...
**Add Child Post**: When viewing any post that has zero children, the circular button shows a "+" icon instead of a chevron. Tapping this button navigates to an empty children view showing "No posts yet" with an "add sub-post" button. The user must tap the button to create the first child post. Anyone can add a child post to any post, except for profile posts where only the profile owner can add children.
...
**Add Post Button**: The "add sub-post" button (using standard button color, lowercase text) appears at the top of child post lists, allowing users to add additional children. Anyone can add children to any post, except for profile posts where only the profile owner sees the button. The button color matches all other UI buttons for visual consistency.
