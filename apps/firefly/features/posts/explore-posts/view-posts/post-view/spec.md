...
**Navigate-to-children button**: Posts with children (or query posts) show a circular button on the right side. This button is positioned so that 3/4 of its radius overlaps the post view (i.e., 1/4 of the button extends beyond the right edge). The button is vertically centered within the post view. The button is a fixed 42pt, hidden when compact, and fades into view as the post expands, becoming tappable only when fully expanded.
