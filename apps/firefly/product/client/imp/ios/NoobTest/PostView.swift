...
struct PostView: View {
    ...
    private var postContent: some View {
        ...
        let shadowRadius = lerp(2, 8, expansionFactor)
        let shadowY = lerp(0, 8, expansionFactor)
        let shadowOpacity = lerp(0.2, 0.375, expansionFactor)

        ZStack(alignment: .topLeading) {
            RoundedRectangle(cornerRadius: 12 * cornerRoundness)
                .fill(Color(red: currentBrightness, green: currentBrightness, blue: currentBrightness))
